import torch
import torch.nn as nn
import pytorch_lightning as pl
import torchaudio
import os
import pathlib
import tqdm
import numpy as np
from collections import defaultdict
from model import (
    EncoderModule,
    ChannelFeatureModule,
    ChannelModule,
    ChannelModule2D,
    MultiScaleSpectralLoss,
    GSTModule,
    load_ssl_model,
    DomainEmbedding,
    LDConditioner,
    ChannelPredictor,
    Projection
)
from utils import (
    manual_logging,
    load_vocoder,
    plot_and_save_mels,
    plot_and_save_mels_all,
)


class PretrainLightningModule(pl.LightningModule):
    """
    Supervised pretraining for low-resource settings.
    This module provides supervised pretraining for the analysis and channel modules.
    """

    def __init__(self, config):
        super().__init__()
        self.save_hyperparameters()
        self.config = config
        if config["general"]["use_gst"]:
            self.encoder = EncoderModule(config)
            self.gst = GSTModule(config)
        else:
            self.encoder = EncoderModule(config, use_channel=True)
            self.channelfeats = ChannelFeatureModule(config)

        self.channel = ChannelModule(config)
        self.vocoder = load_vocoder(config)

        self.criteria_a = MultiScaleSpectralLoss(config)
        if "feature_loss" in config["train"]:
            if config["train"]["feature_loss"]["type"] == "mae":
                self.criteria_b = torch.nn.L1Loss()
            else:
                self.criteria_b = torch.nn.MSELoss()
        else:
            self.criteria = torch.nn.L1Loss()
        self.alpha = config["train"]["alpha"]

    def forward(self, melspecs, wavsaux):
        if self.config["general"]["use_gst"]:
            enc_out = self.encoder(melspecs.unsqueeze(1).transpose(2, 3))
            chfeats = self.gst(melspecs.transpose(1, 2))
        else:
            enc_out, enc_hidden = self.encoder(melspecs.unsqueeze(1).transpose(2, 3))
            chfeats = self.channelfeats(enc_hidden)
        enc_out = enc_out.squeeze(1).transpose(1, 2)
        wavsdeg = self.channel(wavsaux, chfeats)
        return enc_out, wavsdeg

    def training_step(self, batch, batch_idx):
        enc_out, wavsdeg = self.forward(batch["melspecs"], batch["wavsaux"])
        loss_recons = self.criteria_a(wavsdeg, batch["wavs"])
        if self.config["general"]["feature_type"] == "melspec":
            loss_encoder = self.criteria_b(enc_out, batch["melspecsaux"])
        elif self.config["general"]["feature_type"] == "vocfeats":
            loss_encoder = self.criteria_b(enc_out, batch["melceps"])
        loss = self.alpha * loss_recons + (1 - self.alpha) * loss_encoder
        self.log(
            "train/total_loss", loss, on_step=True, on_epoch=True, prog_bar=True, logger=True
        )
        self.log(
            "train/loss_recons",
            loss_recons,
            on_step=True,
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        self.log(
            "train/loss_encoder",
            loss_encoder,
            on_step=True,
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        return loss

    def validation_step(self, batch, batch_idx):
        enc_out, wavsdeg = self.forward(batch["melspecs"], batch["wavsaux"])
        loss_recons = self.criteria_a(wavsdeg, batch["wavs"])
        if self.config["general"]["feature_type"] == "melspec":
            val_aux_feats = batch["melspecsaux"]
            feats_name = "melspec"
            loss_encoder = self.criteria_b(enc_out, val_aux_feats)
        elif self.config["general"]["feature_type"] == "vocfeats":
            val_aux_feats = batch["melceps"]
            feats_name = "melcep"
            loss_encoder = self.criteria_b(enc_out, val_aux_feats)
        loss = self.alpha * loss_recons + (1 - self.alpha) * loss_encoder
        logger_img_dict = {
            "val_src_melspec": batch["melspecs"],
            "val_pred_{}".format(feats_name): enc_out,
            "val_aux_{}".format(feats_name): val_aux_feats,
        }
        logger_wav_dict = {
            "val_src_wav": batch["wavs"],
            "val_pred_wav": wavsdeg,
            "val_aux_wav": batch["wavsaux"],
        }
        return {
            "val_loss": loss,
            "val_loss_recons": loss_recons,
            "val_loss_encoder": loss_encoder,
            "logger_dict": [logger_img_dict, logger_wav_dict],
        }

    def validation_epoch_end(self, outputs):
        val_loss = torch.stack([out["val_loss"] for out in outputs]).mean().item()
        val_loss_recons = (
            torch.stack([out["val_loss_recons"] for out in outputs]).mean().item()
        )
        val_loss_encoder = (
            torch.stack([out["val_loss_encoder"] for out in outputs]).mean().item()
        )
        self.log("val/total_loss", val_loss, on_epoch=True, prog_bar=True, logger=True)
        self.log(
            "val/loss_recons",
            val_loss_recons,
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        self.log(
            "val/loss_encoder",
            val_loss_encoder,
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        self.tflogger(logger_dict=outputs[-1]["logger_dict"][0], data_type="image")
        self.tflogger(logger_dict=outputs[-1]["logger_dict"][1], data_type="audio")

    def test_step(self, batch, batch_idx):
        enc_out, wavsdeg = self.forward(batch["melspecs"], batch["wavsaux"])
        if self.config["general"]["feature_type"] == "melspec":
            enc_feats = enc_out
            enc_feats_aux = batch["melspecsaux"]
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_feats = torch.cat((batch["f0s"].unsqueeze(1), enc_out), dim=1)
            enc_feats_aux = torch.cat(
                (batch["f0s"].unsqueeze(1), batch["melceps"]), dim=1
            )
        recons_wav = self.vocoder(enc_feats_aux).squeeze(1)
        remas = self.vocoder(enc_feats).squeeze(1)
        if self.config["general"]["feature_type"] == "melspec":
            enc_feats_input = batch["melspecs"]
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_feats_input = torch.cat(
                (batch["f0s"].unsqueeze(1), batch["melcepssrc"]), dim=1
            )
        input_recons = self.vocoder(enc_feats_input).squeeze(1)
        if "wavsaux" in batch:
            gt_wav = batch["wavsaux"]
        else:
            gt_wav = None
        return {
            "wavname": batch["wavname"],
            "reconstructed": recons_wav,
            "remastered": remas,
            "channeled": wavsdeg,
            "groundtruth": gt_wav,
            "input": batch["wavs"],
            "input_recons": input_recons,
        }

    def test_epoch_end(self, outputs):
        wav_dir = (
            pathlib.Path(self.logger.experiment.log_dir).parent.parent / "test_wavs"
        )
        mel_dir = (
            pathlib.Path(self.logger.experiment.log_dir).parent.parent / "test_mels"
        )
        print("Saving mel spectrogram plots ...")
        for out in tqdm.tqdm(outputs):
            plot_keys = []
            for key in [
                "reconstructed",
                "remastered",
                "channeled",
                "input",
                "input_recons",
                "groundtruth",
            ]:
                os.makedirs(wav_dir / key, exist_ok=True)
                os.makedirs(mel_dir / key, exist_ok=True)
                if out[key] != None:
                    plot_keys.append(key)
                    torchaudio.save(
                        wav_dir / key / f'{out["wavname"][0]}.wav',
                        out[key][0, ...].unsqueeze(0).cpu(),
                        sample_rate=self.config["preprocess"]["sampling_rate"],
                        channels_first=True,
                    )
                    plot_and_save_mels(
                        out[key][0, ...].cpu(),
                        mel_dir / key / f'{out["wavname"][0]}.png',
                        self.config,
                    )

    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            self.parameters(), lr=self.config["train"]["learning_rate"]
        )
        lr_scheduler_config = {
            "scheduler": torch.optim.lr_scheduler.ReduceLROnPlateau(
                optimizer, mode="min", factor=0.5, min_lr=1e-5, verbose=True
            ),
            "interval": "epoch",
            "frequency": 3,
            "monitor": "val/total_loss",
        }
        return {"optimizer": optimizer, "lr_scheduler": lr_scheduler_config}

    def tflogger(self, logger_dict, data_type):
        for lg in self.loggers:
            if type(lg).__name__ == "TensorBoardLogger":
                tensorboard = lg.experiment
        for key in logger_dict.keys():
            manual_logging(
                logger=tensorboard,
                item=logger_dict[key],
                idx=0,
                tag=key,
                global_step=self.global_step,
                data_type=data_type,
                config=self.config,
            )


class SSLBaseModule(pl.LightningModule):
    def __init__(self, config):
        super().__init__()
        self.save_hyperparameters()
        self.config = config
        if config["general"]["use_gst"]:
            self.encoder = EncoderModule(config)
            self.gst = GSTModule(config)
        else:
            self.encoder = EncoderModule(config, use_channel=True)
            self.channelfeats = ChannelFeatureModule(config)
        self.channel = ChannelModule(config)

        if config["train"]["load_pretrained"]:
            pre_model = PretrainLightningModule.load_from_checkpoint(
                checkpoint_path=config["train"]["pretrained_path"]
            )
            self.encoder.load_state_dict(pre_model.encoder.state_dict(), strict=False)
            self.channel.load_state_dict(pre_model.channel.state_dict(), strict=False)
            if config["general"]["use_gst"]:
                self.gst.load_state_dict(pre_model.gst.state_dict(), strict=False)
            else:
                self.channelfeats.load_state_dict(
                    pre_model.channelfeats.state_dict(), strict=False
                )
        
        if config["train"]["perceptual_loss"]["enabled"]:
            self.utmos = UTMOSLightningModule.load_from_checkpoint(
                "epoch=3-step=7459.ckpt"
            ).eval()
            self.plos_weight = config["train"]["perceptual_loss"]["weight"]
            for param in self.utmos.parameters():
                param.requires_grad = False
            self.resampler = torchaudio.transforms.Resample(
                orig_freq=config["preprocess"]["sampling_rate"],
                new_freq=16000,
                resampling_method="sinc_interpolation",
                lowpass_filter_width=6,
                dtype=torch.float32)

        if config["train"]["channel_loss"]["enabled"]:
            self.chloss_weight = config["train"]["channel_loss"]["weight"]
            self.chloss = nn.MSELoss()
            self.chpredictor = ChannelPredictor()
        
        self.vocoder = load_vocoder(config)
        self.criteria = self.get_loss_function(config)

    def training_step(self, batch, batch_idx):
        raise NotImplementedError()

    def validation_step(self, batch, batch_idx):
        raise NotImplementedError()

    def validation_epoch_end(self, outputs):
        raise NotImplementedError()

    def configure_optimizers(self):
        raise NotImplementedError()

    def get_loss_function(self, config):
        raise NotImplementedError()

    def forward(self, melspecs, f0s=None):
        if self.config["general"]["use_gst"]:
            enc_out = self.encoder(melspecs.unsqueeze(1).transpose(2, 3))
            chfeats = self.gst(melspecs.transpose(1, 2))
        else:
            enc_out, enc_hidden = self.encoder(melspecs.unsqueeze(1).transpose(2, 3))
            chfeats = self.channelfeats(enc_hidden)
        enc_out = enc_out.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            enc_feats = enc_out
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_feats = torch.cat((f0s.unsqueeze(1), enc_out), dim=1)
        remas = self.vocoder(enc_feats).squeeze(1)
        wavsdeg = self.channel(remas, chfeats)
        return remas, wavsdeg

    def test_step(self, batch, batch_idx):
        if self.config["general"]["use_gst"]:
            enc_out = self.encoder(batch["melspecs"].unsqueeze(1).transpose(2, 3))
            chfeats = self.gst(batch["melspecs"].transpose(1, 2))
        else:
            enc_out, enc_hidden = self.encoder(
                batch["melspecs"].unsqueeze(1).transpose(2, 3)
            )
            chfeats = self.channelfeats(enc_hidden)
        enc_out = enc_out.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            enc_feats = enc_out
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_feats = torch.cat((batch["f0s"].unsqueeze(1), enc_out), dim=1)
        remas = self.vocoder(enc_feats).squeeze(1)
        wavsdeg = self.channel(remas, chfeats)
        if self.config["general"]["feature_type"] == "melspec":
            enc_feats_input = batch["melspecs"]
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_feats_input = torch.cat(
                (batch["f0s"].unsqueeze(1), batch["melcepssrc"]), dim=1
            )
        input_recons = self.vocoder(enc_feats_input).squeeze(1)
        if "wavsaux" in batch:
            gt_wav = batch["wavsaux"]
            if self.config["general"]["feature_type"] == "melspec":
                enc_feats_aux = batch["melspecsaux"]
            elif self.config["general"]["feature_type"] == "vocfeats":
                enc_feats_aux = torch.cat(
                    (batch["f0s"].unsqueeze(1), batch["melceps"]), dim=1
                )
            recons_wav = self.vocoder(enc_feats_aux).squeeze(1)
        else:
            gt_wav = None
            recons_wav = None
        return {
            "wavname": batch["wavname"],
            "reconstructed": recons_wav,
            "remastered": remas,
            "channeled": wavsdeg,
            "input": batch["wavs"],
            "input_recons": input_recons,
            "groundtruth": gt_wav,
        }

    def test_epoch_end(self, outputs):
        wav_dir = (
            pathlib.Path(self.logger.experiment.log_dir).parent.parent / "test_wavs"
        )
        mel_dir = (
            pathlib.Path(self.logger.experiment.log_dir).parent.parent / "test_mels"
        )
        print("Saving mel spectrogram plots ...")
        for out in tqdm.tqdm(outputs):
            plot_keys = []
            for key in [
                "reconstructed",
                "remastered",
                "channeled",
                "input",
                "input_recons",
                "groundtruth",
            ]:
                os.makedirs(wav_dir / key, exist_ok=True)
                os.makedirs(mel_dir / key, exist_ok=True)
                if out[key] != None:
                    plot_keys.append(key)
                    torchaudio.save(
                        wav_dir / key / f'{out["wavname"][0]}.wav',
                        out[key][0, ...].unsqueeze(0).cpu(),
                        sample_rate=self.config["preprocess"]["sampling_rate"],
                        channels_first=True,
                    )
                    plot_and_save_mels(
                        out[key][0, ...].cpu(),
                        mel_dir / key / f'{out["wavname"][0]}.png',
                        self.config,
                    )

    def tflogger(self, logger_dict, data_type):
        for lg in self.loggers:
            if type(lg).__name__ == "TensorBoardLogger":
                tensorboard = lg.experiment
        for key in logger_dict.keys():
            manual_logging(
                logger=tensorboard,
                item=logger_dict[key],
                idx=0,
                tag=key,
                global_step=self.global_step,
                data_type=data_type,
                config=self.config,
            )


class SSLStepLightningModule(SSLBaseModule):
    """
    Self-supervised speech restoration model
    with fine-tuning supervisedly pretrained model,
    correspond to ``SSL-pre'' in the paper.

    This module provises step-wise learning, which only trains the channel module at early epochs
    and then only train the analysis module ar later epochs to stabilize the traininig.
    """

    def __init__(self, config):
        super().__init__(config)
        if config["train"]["fix_channel"]:
            for param in self.channel.parameters():
                param.requires_grad = False

    def training_step(self, batch, batch_idx, optimizer_idx):
        if self.config["general"]["use_gst"]:
            enc_out = self.encoder(batch["melspecs"].unsqueeze(1).transpose(2, 3))
            chfeats = self.gst(batch["melspecs"].transpose(1, 2))
        else:
            enc_out, enc_hidden = self.encoder(
                batch["melspecs"].unsqueeze(1).transpose(2, 3)
            )
            chfeats = self.channelfeats(enc_hidden)
        enc_out = enc_out.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            enc_feats = enc_out
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_feats = torch.cat((batch["f0s"].unsqueeze(1), enc_out), dim=1)
        remas = self.vocoder(enc_feats).squeeze(1)
        wavsdeg = self.channel(remas, chfeats)
        loss = self.criteria(wavsdeg, batch["wavs"])

        if self.config["train"]["perceptual_loss"]["enabled"]:
            remas_resampled = self.resampler(remas)
            bs = remas_resampled.shape[0]
            inp_utmos = {
                'wav': remas_resampled.unsqueeze(1),
                'domains': torch.zeros(bs, dtype=torch.int).to(self.device),
                'judge_id': torch.ones(bs, dtype=torch.int).to(self.device)*288
            }
            avg_score = self.utmos(inp_utmos).mean(dim=(0,1)).squeeze(-1).squeeze(-1).squeeze(-1)
            perceptual_loss = -self.plos_weight * avg_score
            loss += perceptual_loss
            self.log(
                "train/perceptual_loss",
                perceptual_loss,
                on_step=True,
                on_epoch=True,
                prog_bar=True,
                logger=True,
            )
        if self.config["train"]["channel_loss"]["enabled"]:
            pred_score = self.chpredictor(chfeats)
            channel_loss = self.chloss_weight * self.chloss(pred_score, batch["labelidx"].float())
            loss += channel_loss
            self.log(
                "train/channel_loss",
                channel_loss,
                on_step=True,
                on_epoch=True,
                prog_bar=True,
                logger=True,
            )
        self.log(
            "train/total_loss",
            loss,
            on_step=True,
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        return loss

    def validation_step(self, batch, batch_idx):
        if self.config["general"]["use_gst"]:
            enc_out = self.encoder(batch["melspecs"].unsqueeze(1).transpose(2, 3))
            chfeats = self.gst(batch["melspecs"].transpose(1, 2))
        else:
            enc_out, enc_hidden = self.encoder(
                batch["melspecs"].unsqueeze(1).transpose(2, 3)
            )
            chfeats = self.channelfeats(enc_hidden)
        enc_out = enc_out.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            enc_feats = enc_out
            feats_name = "melspec"
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_feats = torch.cat((batch["f0s"].unsqueeze(1), enc_out), dim=1)
            feats_name = "melcep"
        remas = self.vocoder(enc_feats).squeeze(1)
        wavsdeg = self.channel(remas, chfeats)
        loss = self.criteria(wavsdeg, batch["wavs"])

        if self.config["train"]["perceptual_loss"]["enabled"]:
            remas_resampled = self.resampler(remas)
            bs = remas_resampled.shape[0]
            inp_utmos = {
                'wav': remas_resampled.unsqueeze(1),
                'domains': torch.zeros(bs, dtype=torch.int).to(self.device),
                'judge_id': torch.ones(bs, dtype=torch.int).to(self.device)*288
            }
            avg_score = self.utmos(inp_utmos).mean(dim=(0,1)).squeeze(-1).squeeze(-1).squeeze(-1)
            perceptual_loss = -self.plos_weight * avg_score
            loss += perceptual_loss

        if self.config["train"]["channel_loss"]["enabled"]:
            pred_score = self.chpredictor(chfeats)
            channel_loss = self.chloss_weight * self.chloss(pred_score, batch["labelidx"])
            loss += channel_loss

        logger_img_dict = {
            "val_src_melspec": batch["melspecs"],
            "val_pred_{}".format(feats_name): enc_out,
        }
        for auxfeats in ["melceps", "melspecsaux"]:
            if auxfeats in batch:
                logger_img_dict["val_aux_{}".format(auxfeats)] = batch[auxfeats]
        logger_wav_dict = {
            "val_src_wav": batch["wavs"],
            "val_remastered_wav": remas,
            "val_pred_wav": wavsdeg,
        }
        if "wavsaux" in batch:
            logger_wav_dict["val_aux_wav"] = batch["wavsaux"]
        d_out = {"val_loss": loss, "logger_dict": [logger_img_dict, logger_wav_dict]}
        if self.config["train"]["perceptual_loss"]["enabled"]:
            d_out["perceptual_loss"] = perceptual_loss
        if self.config["train"]["channel_loss"]["enabled"]:
            d_out["channel_loss"] = channel_loss
        return d_out

    def validation_epoch_end(self, outputs):
        self.log(
            "val/total_loss",
            torch.stack([out["val_loss"] for out in outputs]).mean().item(),
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        if self.config["train"]["perceptual_loss"]["enabled"]:
            self.log(
                "val/perceptual_loss",
                torch.stack([out["perceptual_loss"] for out in outputs]).mean().item(),
                on_epoch=True,
                prog_bar=True,
                logger=True,
            )
        if self.config["train"]["channel_loss"]["enabled"]:
            self.log(
                "val/channel_loss",
                torch.stack([out["channel_loss"] for out in outputs]).mean().item(),
                on_epoch=True,
                prog_bar=True,
                logger=True,
            )
        self.tflogger(logger_dict=outputs[-1]["logger_dict"][0], data_type="image")
        self.tflogger(logger_dict=outputs[-1]["logger_dict"][1], data_type="audio")

    def optimizer_step(
        self,
        epoch,
        batch_idx,
        optimizer,
        optimizer_idx,
        optimizer_closure,
        on_tpu=False,
        using_native_amp=False,
        using_lbfgs=False,
    ):
        if epoch < self.config["train"]["epoch_channel"]:
            if optimizer_idx == 0:
                optimizer.step(closure=optimizer_closure)
            elif optimizer_idx == 1:
                optimizer_closure()
        else:
            if optimizer_idx == 0:
                optimizer_closure()
            elif optimizer_idx == 1:
                optimizer.step(closure=optimizer_closure)

    def configure_optimizers(self):
        if self.config["train"]["fix_channel"]:
            if self.config["general"]["use_gst"]:
                optimizer_channel = torch.optim.Adam(
                    self.gst.parameters(), lr=self.config["train"]["learning_rate"]
                )
            else:
                optimizer_channel = torch.optim.Adam(
                    self.channelfeats.parameters(),
                    lr=self.config["train"]["learning_rate"],
                )
            optimizer_encoder = torch.optim.Adam(
                self.encoder.parameters(), lr=self.config["train"]["learning_rate"]
            )
        else:
            if self.config["general"]["use_gst"]:
                optimizer_channel = torch.optim.Adam(
                    [
                        {"params": self.channel.parameters()},
                        {"params": self.gst.parameters()},
                    ],
                    lr=self.config["train"]["learning_rate"],
                )
            else:
                optimizer_channel = torch.optim.Adam(
                    [
                        {"params": self.channel.parameters()},
                        {"params": self.channelfeats.parameters()},
                    ],
                    lr=self.config["train"]["learning_rate"],
                )
            optimizer_encoder = torch.optim.Adam(
                self.encoder.parameters(), lr=self.config["train"]["learning_rate"]
            )
        optimizers = [optimizer_channel, optimizer_encoder]
        schedulers = [
            {
                "scheduler": torch.optim.lr_scheduler.ReduceLROnPlateau(
                    optimizers[0], mode="min", factor=0.5, min_lr=1e-5, verbose=True
                ),
                "interval": "epoch",
                "frequency": 3,
                "monitor": "val_loss",
            },
            {
                "scheduler": torch.optim.lr_scheduler.ReduceLROnPlateau(
                    optimizers[1], mode="min", factor=0.5, min_lr=1e-5, verbose=True
                ),
                "interval": "epoch",
                "frequency": 3,
                "monitor": "val_loss",
            },
        ]
        return optimizers, schedulers

    def get_loss_function(self, config):
        return MultiScaleSpectralLoss(config)


class SSLDualLightningModule(SSLBaseModule):
    """
    Self-supervised speech restoration model with dual learning,
    correspond to ``SSL-dual'' or ``SSL-dual-pre'' in the paper.

    This module provises dual-learning.
    In addition to the basic training framework, we introduce a training task that
    propagates information in the reverse direction.
    """

    def __init__(self, config):
        super().__init__(config)
        if config["train"]["fix_channel"]:
            for param in self.channel.parameters():
                param.requires_grad = False
        self.spec_module = torchaudio.transforms.MelSpectrogram(
            sample_rate=config["preprocess"]["sampling_rate"],
            n_fft=config["preprocess"]["fft_length"],
            win_length=config["preprocess"]["frame_length"],
            hop_length=config["preprocess"]["frame_shift"],
            f_min=config["preprocess"]["fmin"],
            f_max=config["preprocess"]["fmax"],
            n_mels=config["preprocess"]["n_mels"],
            power=1,
            center=True,
            norm="slaney",
            mel_scale="slaney",
        )
        self.beta = config["train"]["beta"]
        self.criteria_a, self.criteria_b = self.get_loss_function(config)

    def training_step(self, batch, batch_idx):
        if self.config["general"]["use_gst"]:
            enc_out = self.encoder(batch["melspecs"].unsqueeze(1).transpose(2, 3))
            chfeats = self.gst(batch["melspecs"].transpose(1, 2))
        else:
            enc_out, enc_hidden = self.encoder(
                batch["melspecs"].unsqueeze(1).transpose(2, 3)
            )
            chfeats = self.channelfeats(enc_hidden)
        enc_out = enc_out.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            enc_feats = enc_out
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_feats = torch.cat((batch["f0s"].unsqueeze(1), enc_out), dim=1)
        remas = self.vocoder(enc_feats).squeeze(1)
        wavsdeg = self.channel(remas, chfeats)
        loss_recons = self.criteria_a(wavsdeg, batch["wavs"])

        self.log(
            "train/loss_recons",
            loss_recons,
            on_step=True,
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )

        with torch.no_grad():
            if self.config["dual"]["use_noise_aug"]:
                wavsdegtask = self.channel(batch["wavstaskaug"], chfeats)
            else:
                wavsdegtask = self.channel(batch["wavstask"], chfeats)
        melspecstask = self.calc_spectrogram(wavsdegtask)
        if self.config["general"]["use_gst"]:
            enc_out_task = self.encoder(melspecstask.unsqueeze(1).transpose(2, 3))
        else:
            enc_out_task, _ = self.encoder(melspecstask.unsqueeze(1).transpose(2, 3))
        enc_out_task = enc_out_task.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            loss_task = self.criteria_b(enc_out_task, batch["melspecstask"])
        elif self.config["general"]["feature_type"] == "vocfeats":
            loss_task = self.criteria_b(enc_out_task, batch["melcepstask"])
        self.log(
            "train/loss_task",
            loss_task,
            on_step=True,
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        loss = self.beta * loss_recons + (1 - self.beta) * loss_task

        if self.config["train"]["perceptual_loss"]["enabled"]:
            remas_resampled = self.resampler(remas)
            bs = remas_resampled.shape[0]
            inp_utmos = {
                'wav': remas_resampled.unsqueeze(1),
                'domains': torch.zeros(bs, dtype=torch.int).to(self.device),
                'judge_id': torch.ones(bs, dtype=torch.int).to(self.device)*288
            }
            avg_score = self.utmos(inp_utmos).mean(dim=(0,1)).squeeze(-1).squeeze(-1).squeeze(-1)
            perceptual_loss = -self.plos_weight * avg_score
            loss += perceptual_loss
            self.log(
                "train/perceptual_loss",
                perceptual_loss,
                on_step=True,
                on_epoch=True,
                prog_bar=True,
                logger=True)

        if self.config["train"]["channel_loss"]["enabled"]:
            pred_score = self.chpredictor(chfeats)
            channel_loss = self.chloss_weight * self.chloss(pred_score, batch["labelidx"].float())
            loss += channel_loss
            self.log(
                "train/channel_loss",
                channel_loss,
                on_step=True,
                on_epoch=True,
                prog_bar=True,
                logger=True)

        self.log(
            "train/total_loss",
            loss,
            on_step=True,
            on_epoch=True,
            prog_bar=True,
            logger=True
        )
        return loss

    def validation_step(self, batch, batch_idx):
        if self.config["general"]["use_gst"]:
            enc_out = self.encoder(batch["melspecs"].unsqueeze(1).transpose(2, 3))
            chfeats = self.gst(batch["melspecs"].transpose(1, 2))
        else:
            enc_out, enc_hidden = self.encoder(
                batch["melspecs"].unsqueeze(1).transpose(2, 3)
            )
            chfeats = self.channelfeats(enc_hidden)
        enc_out = enc_out.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            enc_feats = enc_out
            feats_name = "melspec"
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_feats = torch.cat((batch["f0s"].unsqueeze(1), enc_out), dim=1)
            feats_name = "melcep"
        remas = self.vocoder(enc_feats).squeeze(1)
        wavsdeg = self.channel(remas, chfeats)
        loss_recons = self.criteria_a(wavsdeg, batch["wavs"])

        if self.config["dual"]["use_noise_aug"]:
            wavsdegtask = self.channel(batch["wavstaskaug"], chfeats)
        else:
            wavsdegtask = self.channel(batch["wavstask"], chfeats)
        melspecstask = self.calc_spectrogram(wavsdegtask)
        if self.config["general"]["use_gst"]:
            enc_out_task = self.encoder(melspecstask.unsqueeze(1).transpose(2, 3))
        else:
            enc_out_task, _ = self.encoder(melspecstask.unsqueeze(1).transpose(2, 3))
        enc_out_task = enc_out_task.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            enc_out_task_truth = batch["melspecstask"]
            loss_task = self.criteria_b(enc_out_task, enc_out_task_truth)
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_out_task_truth = batch["melcepstask"]
            loss_task = self.criteria_b(enc_out_task, enc_out_task_truth)
        loss = self.beta * loss_recons + (1 - self.beta) * loss_task

        if self.config["train"]["perceptual_loss"]["enabled"]:
            remas_resampled = self.resampler(remas)
            bs = remas_resampled.shape[0]
            inp_utmos = {
                'wav': remas_resampled.unsqueeze(1),
                'domains': torch.zeros(bs, dtype=torch.int).to(self.device),
                'judge_id': torch.ones(bs, dtype=torch.int).to(self.device)*288
            }
            avg_score = self.utmos(inp_utmos).mean(dim=(0,1)).squeeze(-1).squeeze(-1).squeeze(-1)
            perceptual_loss = -self.plos_weight * avg_score
            loss += perceptual_loss

        if self.config["train"]["channel_loss"]["enabled"]:
            pred_score = self.chpredictor(chfeats)
            channel_loss = self.chloss_weight * self.chloss(pred_score, batch["labelidx"].float())
            loss += channel_loss

        logger_img_dict = {
            "val_src_melspec": batch["melspecs"],
            "val_pred_{}".format(feats_name): enc_out,
            "val_truth_{}_task".format(feats_name): enc_out_task_truth,
            "val_pred_{}_task".format(feats_name): enc_out_task,
        }
        for auxfeats in ["melceps", "melspecsaux"]:
            if auxfeats in batch:
                logger_img_dict["val_aux_{}".format(auxfeats)] = batch[auxfeats]
        logger_wav_dict = {
            "val_src_wav": batch["wavs"],
            "val_remastered_wav": remas,
            "val_pred_wav": wavsdeg,
            "val_truth_wavtask": batch["wavstask"],
            "val_deg_wavtask": wavsdegtask,
        }
        if "wavsaux" in batch:
            logger_wav_dict["val_aux_wav"] = batch["wavsaux"]
        if "wavstaskaug" in batch:
            logger_wav_dict["val_aug_wavtask"] = batch["wavstaskaug"]

        d_out = {
            "val_loss": loss,
            "val_loss_recons": loss_recons,
            "val_loss_task": loss_task,
            "logger_dict": [logger_img_dict, logger_wav_dict],
        }
        if self.config["train"]["perceptual_loss"]["enabled"]:
            d_out["val_perceptual_loss"] = perceptual_loss
        if self.config["train"]["channel_loss"]["enabled"]:
            d_out["val_channel_loss"] = channel_loss
        return d_out

    def validation_epoch_end(self, outputs):
        self.log(
            "val/total_loss",
            torch.stack([out["val_loss"] for out in outputs]).mean().item(),
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        self.log(
            "val/loss_recons",
            torch.stack([out["val_loss_recons"] for out in outputs]).mean().item(),
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        self.log(
            "val/loss_task",
            torch.stack([out["val_loss_task"] for out in outputs]).mean().item(),
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        if self.config["train"]["perceptual_loss"]["enabled"]:
            self.log(
                "val/perceptual_loss",
                torch.stack([out["val_perceptual_loss"] for out in outputs]).mean().item(),
                on_epoch=True,
                prog_bar=True,
                logger=True,
            )
        if self.config["train"]["channel_loss"]["enabled"]:
            self.log(
                "val/channel_loss",
                torch.stack([out["val_channel_loss"] for out in outputs]).mean().item(),
                on_epoch=True,
                prog_bar=True,
                logger=True,
            )
        self.tflogger(logger_dict=outputs[-1]["logger_dict"][0], data_type="image")
        self.tflogger(logger_dict=outputs[-1]["logger_dict"][1], data_type="audio")

    def test_step(self, batch, batch_idx):
        if self.config["general"]["use_gst"]:
            enc_out = self.encoder(batch["melspecs"].unsqueeze(1).transpose(2, 3))
            chfeats = self.gst(batch["melspecs"].transpose(1, 2))
        else:
            enc_out, enc_hidden = self.encoder(
                batch["melspecs"].unsqueeze(1).transpose(2, 3)
            )
            chfeats = self.channelfeats(enc_hidden)
        enc_out = enc_out.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            enc_feats = enc_out
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_feats = torch.cat((batch["f0s"].unsqueeze(1), enc_out), dim=1)
        remas = self.vocoder(enc_feats).squeeze(1)
        wavsdeg = self.channel(remas, chfeats)
        if self.config["general"]["feature_type"] == "melspec":
            enc_feats_input = batch["melspecs"]
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_feats_input = torch.cat(
                (batch["f0s"].unsqueeze(1), batch["melcepssrc"]), dim=1
            )
        input_recons = self.vocoder(enc_feats_input).squeeze(1)

        if "wavsaux" in batch:
            gt_wav = batch["wavsaux"]
            if self.config["general"]["feature_type"] == "melspec":
                enc_feats_aux = batch["melspecsaux"]
            elif self.config["general"]["feature_type"] == "vocfeats":
                enc_feats_aux = torch.cat(
                    (batch["f0s"].unsqueeze(1), batch["melceps"]), dim=1
                )
            recons_wav = self.vocoder(enc_feats_aux).squeeze(1)
        else:
            gt_wav = None
            recons_wav = None
        d_out =  {
            "wavname": batch["wavname"],
            "reconstructed": recons_wav,
            "remastered": remas,
            "channeled": wavsdeg,
            "input": batch["wavs"],
            "input_recons": input_recons,
            "groundtruth": gt_wav
        }
        if "dump_chfeats" in self.config["general"] and self.config["general"]["dump_chfeats"]:
            d_out["chfeats"] = chfeats
            d_out["labelname"] = batch["labelname"]
        return d_out

    def test_epoch_end(self, outputs):
        wav_dir = (
            pathlib.Path(self.logger.experiment.log_dir).parent.parent / "test_wavs"
        )
        mel_dir = (
            pathlib.Path(self.logger.experiment.log_dir).parent.parent / "test_mels"
        )
        print("Saving mel spectrogram plots ...")
        for out in tqdm.tqdm(outputs):
            plot_keys = []
            for key in [
                "reconstructed",
                "remastered",
                "channeled",
                "input",
                "input_recons",
                "groundtruth",
            ]:
                os.makedirs(wav_dir / key, exist_ok=True)
                os.makedirs(mel_dir / key, exist_ok=True)
                if out[key] != None:
                    plot_keys.append(key)
                    torchaudio.save(
                        wav_dir / key / f'{out["wavname"][0]}.wav',
                        out[key][0, ...].unsqueeze(0).cpu(),
                        sample_rate=self.config["preprocess"]["sampling_rate"],
                        channels_first=True,
                    )
                    plot_and_save_mels(
                        out[key][0, ...].cpu(),
                        mel_dir / key / f'{out["wavname"][0]}.png',
                        self.config,
                    )
        if "dump_chfeats" in self.config["general"] and self.config["general"]["dump_chfeats"]:
            print("Dumping channel features ...")
            chf_dir = (
                pathlib.Path(self.logger.experiment.log_dir).parent.parent / "chfeats"
            )
            os.makedirs(chf_dir, exist_ok=True)
            out_chfeats_dict = defaultdict(list)
            for out in tqdm.tqdm(outputs):
                chfeats_np = out["chfeats"].cpu().numpy()
                out_chfeats_dict[out["labelname"][0]].append(chfeats_np)
            for key, value in out_chfeats_dict.items():
                np.save(chf_dir / f"{key}.npy", np.concatenate(value, axis=0))

    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            self.parameters(), lr=self.config["train"]["learning_rate"]
        )
        lr_scheduler_config = {
            "scheduler": torch.optim.lr_scheduler.ReduceLROnPlateau(
                optimizer, mode="min", factor=0.5, min_lr=1e-5, verbose=True
            ),
            "interval": "epoch",
            "frequency": 3,
            "monitor": "val/total_loss",
        }
        return {"optimizer": optimizer, "lr_scheduler": lr_scheduler_config}

    def calc_spectrogram(self, wav):
        specs = self.spec_module(wav)
        log_spec = torch.log(
            torch.clamp_min(specs, self.config["preprocess"]["min_magnitude"])
            * self.config["preprocess"]["comp_factor"]
        ).to(torch.float32)
        return log_spec

    def get_loss_function(self, config):
        if config["train"]["feature_loss"]["type"] == "mae":
            feature_loss = torch.nn.L1Loss()
        else:
            feature_loss = torch.nn.MSELoss()
        return MultiScaleSpectralLoss(config), feature_loss


class SemiDualLightningModule(SSLDualLightningModule):
    """
    Semi-supervised speech restoration model
    correspond to ``Semi'' in the paper.

    This module uses dual-learning.
    In addition to the basic training framework, we introduce a training task that
    propagates information in the reverse direction.
    It also uses supervised learning to train the analysis and channel modules.
    """

    def __init__(self, config):
        super().__init__(config)
        if "use_semi_recons" in config["train"]:
            self.use_semi_recons = config["train"]["use_semi_recons"]
        else:
            self.use_semi_recons = False

    def training_step(self, batch, batch_idx):
        batch_ssl = batch["ssl"]
        batch_semi = batch["semi"]

        # ssl loop
        enc_out_ssl, enc_hidden_ssl = self.encoder(
            batch_ssl["melspecs"].unsqueeze(1).transpose(2, 3)
        )
        chfeats_ssl = self.channelfeats(enc_hidden_ssl)
        enc_out_ssl = enc_out_ssl.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            enc_feats_ssl = enc_out_ssl
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_feats_ssl = torch.cat((batch_ssl["f0s"].unsqueeze(1), enc_out_ssl), dim=1)
        remas_ssl = self.vocoder(enc_feats_ssl).squeeze(1)
        wavsdeg_ssl = self.channel(remas_ssl, chfeats_ssl)
        loss_recons_ssl = self.criteria_a(wavsdeg_ssl, batch_ssl["wavs"])

        self.log(
            "train/loss_recons_ssl",
            loss_recons_ssl,
            on_step=True,
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )

        with torch.no_grad():
            if self.config["dual"]["use_noise_aug"]:
                wavsdegtask_ssl = self.channel(batch_ssl["wavstaskaug"], chfeats_ssl)
            else:
                wavsdegtask_ssl = self.channel(batch_ssl["wavstask"], chfeats_ssl)
        melspecstask_ssl = self.calc_spectrogram(wavsdegtask_ssl)
        if self.config["general"]["use_gst"]:
            enc_out_task_ssl = self.encoder(melspecstask_ssl.unsqueeze(1).transpose(2, 3))
        else:
            enc_out_task_ssl, _ = self.encoder(melspecstask_ssl.unsqueeze(1).transpose(2, 3))
        enc_out_task_ssl = enc_out_task_ssl.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            loss_task_ssl = self.criteria_b(enc_out_task_ssl, batch_ssl["melspecstask"])
        elif self.config["general"]["feature_type"] == "vocfeats":
            loss_task_ssl = self.criteria_b(enc_out_task_ssl, batch_ssl["melcepstask"])
        self.log(
            "train/loss_task_ssl",
            loss_task_ssl,
            on_step=True,
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        loss = self.beta * loss_recons_ssl + (1 - self.beta) * loss_task_ssl

        # supervised loop
        enc_out_semi, enc_hidden_semi = self.encoder(
            batch_semi["melspecs"].unsqueeze(1).transpose(2, 3)
        )
        enc_out_semi = enc_out_semi.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            loss_encoder_semi = self.criteria_b(enc_out_semi, batch_semi["melspecsaux"])
        elif self.config["general"]["feature_type"] == "vocfeats":
            loss_encoder_semi = self.criteria_b(enc_out_semi, batch_semi["melceps"])

        if self.use_semi_recons:
            chfeats_semi = self.channelfeats(enc_hidden_semi)
            wavsdeg_semi = self.channel(batch_semi["wavsaux"], chfeats_semi)
            loss_recons_semi = self.criteria_a(wavsdeg_semi, batch_semi["wavs"])
            self.log(
                "train/loss_recons_semi",
                loss_recons_semi,
                on_step=True,
                on_epoch=True,
                prog_bar=True,
                logger=True,
            )
            loss += (self.beta*loss_recons_semi + (1 - self.beta)*loss_encoder_semi)
        else:
            loss += loss_encoder_semi

        self.log(
            "train/loss_encoder_semi",
            loss_encoder_semi,
            on_step=True,
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )

        if self.config["train"]["perceptual_loss"]["enabled"]:
            remas_resampled = self.resampler(remas_ssl)
            bs = remas_resampled.shape[0]
            inp_utmos = {
                'wav': remas_resampled.unsqueeze(1),
                'domains': torch.zeros(bs, dtype=torch.int).to(self.device),
                'judge_id': torch.ones(bs, dtype=torch.int).to(self.device)*288
            }
            avg_score = self.utmos(inp_utmos).mean(dim=(0,1)).squeeze(-1).squeeze(-1).squeeze(-1)
            perceptual_loss = -self.plos_weight * avg_score
            loss += perceptual_loss
            self.log(
                "train/perceptual_loss",
                perceptual_loss,
                on_step=True,
                on_epoch=True,
                prog_bar=True,
                logger=True)

        if self.config["train"]["channel_loss"]["enabled"]:
            pred_score_ssl = self.chpredictor(chfeats_ssl)
            chfeats_semi = self.channelfeats(enc_hidden_semi)
            pred_score_semi = self.chpredictor(chfeats_semi)
            channel_loss_ssl = self.chloss_weight * self.chloss(pred_score_ssl, batch_ssl["labelidx"].float())
            channel_loss_semi = self.chloss_weight * self.chloss(pred_score_semi, batch_semi["labelidx"].float())
            loss += (channel_loss_ssl + channel_loss_semi)
            self.log(
                "train/channel_loss_ssl",
                channel_loss_ssl,
                on_step=True,
                on_epoch=True,
                prog_bar=True,
                logger=True)
            self.log(
                "train/channel_loss_semi",
                channel_loss_semi,
                on_step=True,
                on_epoch=True,
                prog_bar=True,
                logger=True)

        self.log(
            "train/total_loss",
            loss,
            on_step=True,
            on_epoch=True,
            prog_bar=True,
            logger=True
        )
        return loss

    def validation_step(self, batch, batch_idx):
        batch_ssl = batch["ssl"]
        batch_semi = batch["semi"]

        enc_out_ssl, enc_hidden_ssl = self.encoder(
            batch_ssl["melspecs"].unsqueeze(1).transpose(2, 3)
        )
        chfeats_ssl = self.channelfeats(enc_hidden_ssl)
        enc_out_ssl = enc_out_ssl.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            enc_feats_ssl = enc_out_ssl
            feats_name = "melspec"
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_feats_ssl = torch.cat((batch_ssl["f0s"].unsqueeze(1), enc_out_ssl), dim=1)
            feats_name = "melcep"
        remas_ssl = self.vocoder(enc_feats_ssl).squeeze(1)
        wavsdeg_ssl = self.channel(remas_ssl, chfeats_ssl)
        loss_recons_ssl = self.criteria_a(wavsdeg_ssl, batch_ssl["wavs"])

        if self.config["dual"]["use_noise_aug"]:
            wavsdegtask_ssl = self.channel(batch_ssl["wavstaskaug"], chfeats_ssl)
        else:
            wavsdegtask_ssl = self.channel(batch_ssl["wavstask"], chfeats_ssl)
        melspecstask_ssl = self.calc_spectrogram(wavsdegtask_ssl)
        enc_out_task_ssl, _ = self.encoder(melspecstask_ssl.unsqueeze(1).transpose(2, 3))
        enc_out_task_ssl = enc_out_task_ssl.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            enc_out_task_truth_ssl = batch_ssl["melspecstask"]
            loss_task_ssl = self.criteria_b(enc_out_task_ssl, enc_out_task_truth_ssl)
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_out_task_truth_ssl = batch_ssl["melcepstask"]
            loss_task_ssl = self.criteria_b(enc_out_task_ssl, enc_out_task_truth_ssl)
        loss = self.beta * loss_recons_ssl + (1 - self.beta) * loss_task_ssl

        # supervised loop
        enc_out_semi, enc_hidden_semi = self.encoder(
            batch_semi["melspecs"].unsqueeze(1).transpose(2, 3)
        )
        enc_out_semi = enc_out_semi.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            loss_encoder_semi = self.criteria_b(enc_out_semi, batch_semi["melspecsaux"])
        elif self.config["general"]["feature_type"] == "vocfeats":
            loss_encoder_semi = self.criteria_b(enc_out_semi, batch_semi["melceps"])

        if self.use_semi_recons:
            chfeats_semi = self.channelfeats(enc_hidden_semi)
            wavsdeg_semi = self.channel(batch_semi["wavsaux"], chfeats_semi)
            loss_recons_semi = self.criteria_a(wavsdeg_semi, batch_semi["wavs"])
            loss += (self.beta*loss_recons_semi + (1 - self.beta)*loss_encoder_semi)
        else:
            loss += loss_encoder_semi

        if self.config["train"]["perceptual_loss"]["enabled"]:
            remas_resampled = self.resampler(remas_ssl)
            bs = remas_resampled.shape[0]
            inp_utmos = {
                'wav': remas_resampled.unsqueeze(1),
                'domains': torch.zeros(bs, dtype=torch.int).to(self.device),
                'judge_id': torch.ones(bs, dtype=torch.int).to(self.device)*288
            }
            avg_score = self.utmos(inp_utmos).mean(dim=(0,1)).squeeze(-1).squeeze(-1).squeeze(-1)
            perceptual_loss = -self.plos_weight * avg_score
            loss += perceptual_loss

        if self.config["train"]["channel_loss"]["enabled"]:
            chfeats_semi = self.channelfeats(enc_hidden_semi)
            pred_score_ssl = self.chpredictor(chfeats_ssl)
            pred_score_semi = self.chpredictor(chfeats_semi)
            channel_loss_ssl = self.chloss_weight * self.chloss(pred_score_ssl, batch_ssl["labelidx"].float())
            channel_loss_semi = self.chloss_weight * self.chloss(pred_score_semi, batch_semi["labelidx"].float())
            loss += (channel_loss_ssl + channel_loss_semi)

        logger_img_dict = {
            "val_src_melspec_ssl": batch_ssl["melspecs"],
            "val_pred_{}_ssl".format(feats_name): enc_out_ssl,
            "val_truth_{}_task_ssl".format(feats_name): enc_out_task_truth_ssl,
            "val_pred_{}_task_ssl".format(feats_name): enc_out_task_ssl,
            "val_src_melspec_semi": batch_semi["melspecs"],
            "val_pred_{}_ssl_semi".format(feats_name): enc_out_semi,
        }
        for auxfeats in ["melceps", "melspecsaux"]:
            if auxfeats in batch_ssl:
                logger_img_dict["val_aux_{}_ssl".format(auxfeats)] = batch_ssl[auxfeats]
            if auxfeats in batch_semi:
                logger_img_dict["val_aux_{}_semi".format(auxfeats)] = batch_semi[auxfeats]
        logger_wav_dict = {
            "val_src_wav_ssl": batch_ssl["wavs"],
            "val_remastered_wav_ssl": remas_ssl,
            "val_pred_wav_ssl": wavsdeg_ssl,
            "val_truth_wavtask_ssl": batch_ssl["wavstask"],
            "val_deg_wavtask_ssl": wavsdegtask_ssl,
            "val_src_wav_semi": batch_semi["wavs"]
        }
        if self.use_semi_recons:
            logger_wav_dict["val_pred_wav_semi"] = wavsdeg_semi
        if "wavsaux" in batch:
            logger_wav_dict["val_aux_wav"] = batch["wavsaux"]
        if "wavstaskaug" in batch:
            logger_wav_dict["val_aug_wavtask"] = batch["wavstaskaug"]

        d_out = {
            "val_loss": loss,
            "val_loss_recons_ssl": loss_recons_ssl,
            "val_loss_task_ssl": loss_task_ssl,
            "val_loss_encoder_semi": loss_encoder_semi,
            "logger_dict": [logger_img_dict, logger_wav_dict],
        }
        if self.use_semi_recons:
            d_out["val_loss_recons_semi"] = loss_recons_semi
        if self.config["train"]["perceptual_loss"]["enabled"]:
            d_out["val_perceptual_loss"] = perceptual_loss
        if self.config["train"]["channel_loss"]["enabled"]:
            d_out["val_channel_loss_ssl"] = channel_loss_ssl
            d_out["val_channel_loss_semi"] = channel_loss_semi
        return d_out

    def validation_epoch_end(self, outputs):
        self.log(
            "val/total_loss",
            torch.stack([out["val_loss"] for out in outputs]).mean().item(),
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        self.log(
            "val/loss_recons_ssl",
            torch.stack([out["val_loss_recons_ssl"] for out in outputs]).mean().item(),
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        self.log(
            "val/loss_task_ssl",
            torch.stack([out["val_loss_task_ssl"] for out in outputs]).mean().item(),
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        if self.use_semi_recons:
            self.log(
                "val/loss_recons_semi",
                torch.stack([out["val_loss_recons_semi"] for out in outputs]).mean().item(),
                on_epoch=True,
                prog_bar=True,
                logger=True,
            )
        self.log(
            "val/loss_encoder_semi",
            torch.stack([out["val_loss_encoder_semi"] for out in outputs]).mean().item(),
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        if self.config["train"]["perceptual_loss"]["enabled"]:
            self.log(
                "val/perceptual_loss",
                torch.stack([out["val_perceptual_loss"] for out in outputs]).mean().item(),
                on_epoch=True,
                prog_bar=True,
                logger=True,
            )
        if self.config["train"]["channel_loss"]["enabled"]:
            self.log(
                "val/channel_loss_ssl",
                torch.stack([out["val_channel_loss_ssl"] for out in outputs]).mean().item(),
                on_epoch=True,
                prog_bar=True,
                logger=True,
            )
            self.log(
                "val/channel_loss_semi",
                torch.stack([out["val_channel_loss_semi"] for out in outputs]).mean().item(),
                on_epoch=True,
                prog_bar=True,
                logger=True,
            )
        self.tflogger(logger_dict=outputs[-1]["logger_dict"][0], data_type="image")
        self.tflogger(logger_dict=outputs[-1]["logger_dict"][1], data_type="audio")

    def test_step(self, batch, batch_idx):
        batch_ssl = batch["ssl"]
        if self.config["general"]["use_gst"]:
            enc_out = self.encoder(batch_ssl["melspecs"].unsqueeze(1).transpose(2, 3))
            chfeats = self.gst(batch_ssl["melspecs"].transpose(1, 2))
        else:
            enc_out, enc_hidden = self.encoder(
                batch_ssl["melspecs"].unsqueeze(1).transpose(2, 3)
            )
            chfeats = self.channelfeats(enc_hidden)
        enc_out = enc_out.squeeze(1).transpose(1, 2)
        if self.config["general"]["feature_type"] == "melspec":
            enc_feats = enc_out
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_feats = torch.cat((batch_ssl["f0s"].unsqueeze(1), enc_out), dim=1)
        remas = self.vocoder(enc_feats).squeeze(1)
        wavsdeg = self.channel(remas, chfeats)
        if self.config["general"]["feature_type"] == "melspec":
            enc_feats_input = batch_ssl["melspecs"]
        elif self.config["general"]["feature_type"] == "vocfeats":
            enc_feats_input = torch.cat(
                (batch_ssl["f0s"].unsqueeze(1), batch_ssl["melcepssrc"]), dim=1
            )
        input_recons = self.vocoder(enc_feats_input).squeeze(1)

        if "wavsaux" in batch_ssl:
            gt_wav = batch_ssl["wavsaux"]
            if self.config["general"]["feature_type"] == "melspec":
                enc_feats_aux = batch_ssl["melspecsaux"]
            elif self.config["general"]["feature_type"] == "vocfeats":
                enc_feats_aux = torch.cat(
                    (batch_ssl["f0s"].unsqueeze(1), batch_ssl["melceps"]), dim=1
                )
            recons_wav = self.vocoder(enc_feats_aux).squeeze(1)
        else:
            gt_wav = None
            recons_wav = None
        d_out = {
            "wavname": batch_ssl["wavname"],
            "reconstructed": recons_wav,
            "remastered": remas,
            "channeled": wavsdeg,
            "input": batch_ssl["wavs"],
            "input_recons": input_recons,
            "groundtruth": gt_wav,
        }
        if "dump_chfeats" in self.config["general"] and self.config["general"]["dump_chfeats"]:
            d_out["chfeats"] = chfeats
            d_out["labelname"] = batch_ssl["labelname"]
        return d_out

    def test_epoch_end(self, outputs):
        wav_dir = (
            pathlib.Path(self.logger.experiment.log_dir).parent.parent / "test_wavs"
        )
        mel_dir = (
            pathlib.Path(self.logger.experiment.log_dir).parent.parent / "test_mels"
        )
        print("Saving mel spectrogram plots ...")
        for out in tqdm.tqdm(outputs):
            plot_keys = []
            for key in [
                "reconstructed",
                "remastered",
                "channeled",
                "input",
                "input_recons",
                "groundtruth",
            ]:
                os.makedirs(wav_dir / key, exist_ok=True)
                os.makedirs(mel_dir / key, exist_ok=True)
                if out[key] != None:
                    plot_keys.append(key)
                    torchaudio.save(
                        wav_dir / key / f'{out["wavname"][0]}.wav',
                        out[key][0, ...].unsqueeze(0).cpu(),
                        sample_rate=self.config["preprocess"]["sampling_rate"],
                        channels_first=True,
                    )
                    plot_and_save_mels(
                        out[key][0, ...].cpu(),
                        mel_dir / key / f'{out["wavname"][0]}.png',
                        self.config,
                    )
        if "dump_chfeats" in self.config["general"] and self.config["general"]["dump_chfeats"]:
            print("Dumping channel features ...")
            chf_dir = (
                pathlib.Path(self.logger.experiment.log_dir).parent.parent / "chfeats"
            )
            os.makedirs(chf_dir, exist_ok=True)
            out_chfeats_dict = defaultdict(list)
            for out in tqdm.tqdm(outputs):
                chfeats_np = out["chfeats"].cpu().numpy()
                out_chfeats_dict[out["labelname"][0]].append(chfeats_np)
            for key, value in out_chfeats_dict.items():
                np.save(chf_dir / f"{key}.npy", np.concatenate(value, axis=0))

    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            self.parameters(), lr=self.config["train"]["learning_rate"]
        )
        lr_scheduler_config = {
            "scheduler": torch.optim.lr_scheduler.ReduceLROnPlateau(
                optimizer, mode="min", factor=0.5, min_lr=1e-5, verbose=True
            ),
            "interval": "epoch",
            "frequency": 3,
            "monitor": "val/total_loss",
        }
        return {"optimizer": optimizer, "lr_scheduler": lr_scheduler_config}

    #def configure_optimizers(self):
    #    optimizer = torch.optim.Adam(
    #        self.parameters(), lr=self.config["train"]["learning_rate"]
    #    )
    #    scheduler = torch.optim.lr_scheduler.ExponentialLR(
    #        optimizer, gamma=0.95)
    #    return [optimizer], [scheduler]

    def calc_spectrogram(self, wav):
        specs = self.spec_module(wav)
        log_spec = torch.log(
            torch.clamp_min(specs, self.config["preprocess"]["min_magnitude"])
            * self.config["preprocess"]["comp_factor"]
        ).to(torch.float32)
        return log_spec

    def get_loss_function(self, config):
        if config["train"]["feature_loss"]["type"] == "mae":
            feature_loss = torch.nn.L1Loss()
        else:
            feature_loss = torch.nn.MSELoss()
        return MultiScaleSpectralLoss(config), feature_loss


class UTMOSLightningModule(pl.LightningModule):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        self.construct_model()
        self.save_hyperparameters()
    
    def construct_model(self):
        self.feature_extractors = nn.ModuleList([
            load_ssl_model(cp_path='wav2vec_small.pt'),
            DomainEmbedding(3,128),
        ])
        output_dim = sum([ feature_extractor.get_output_dim() for feature_extractor in self.feature_extractors])
        output_layers = [
            LDConditioner(judge_dim=128,num_judges=3000,input_dim=output_dim)
        ]
        output_dim = output_layers[-1].get_output_dim()
        output_layers.append(
            Projection(hidden_dim=2048,activation=torch.nn.ReLU(),range_clipping=False,input_dim=output_dim)

        )

        self.output_layers = nn.ModuleList(output_layers)

    def forward(self, inputs):
        outputs = {}
        for feature_extractor in self.feature_extractors:
            outputs.update(feature_extractor(inputs))
        x = outputs
        for output_layer in self.output_layers:
            x = output_layer(x,inputs)
        return x