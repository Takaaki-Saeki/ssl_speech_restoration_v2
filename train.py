...
def get_arg():
    ...
    parser.add_argument(
        ...
        choices=[
            "pretrain", "ssl-step", "ssl-dual", "semi"
        ]
    )
    ...
    parser.add_argument("--channel_kernel_size", default=None, type=int)
    parser.add_argument(
    ...
    parser.add_argument("--noise_dir", default=None, type=pathlib.Path)
...
def train(args, config, output_path):
    ...
    if config["train"]["early_stopping"]:
        earlystop_callback = EarlyStopping(
            monitor="val/total_loss", min_delta=0.0, patience=15, mode="min"
        )
    ...
    trainer = Trainer(
        ...
        log_every_n_steps=config["train"]["logger_step"],
    ...
    if config["general"]["stage"] == "pretrain":
        ...
        model = SemiDualLightningModule(config)
    else:
